...
- Loads failed reports from the database and attempts to re-scrape them.
- Updates the database and logs results.
"""
...
def get_holdings(report_url, manager, quarter, retries=3):
    """
    Fetches holdings data for a given report URL.
    Attempts to fetch data via JSON API or falls back to HTML parsing.
    """
    try:
...
if __name__=="__main__":
    ...
    for url, manager, quarter in failed:
        ...
        if holdings:
            ...
            # Mark empty reports as scraped_empty to avoid endless retries
            cur.execute("DELETE FROM failed_reports WHERE ReportLink=?", (url,))
            cur.execute("INSERT INTO scrape_log (ReportLink, Status) VALUES (?, 'scraped_empty')", (url,))
            log(f"No holdings found (empty report): {url}")
        con.commit()
